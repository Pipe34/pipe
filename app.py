...
FILES ={
    ...
    "Geothemal":("17 installed-geothermal-capacity.csv","Geothermal Capacity")
}
...
def index(): 
    ...
    #--------------- AQUÍ INICIA EL : Gráfico de torta---------------
    ...
    wind_percetage = df['Wind (% electricity)'].values[0]
    solar_percetage = df['Solar (% electricity)'].values[0]
    hydro_percentage = df['Hydro (% electricity)'].values[0]

    total_renewables = wind_percetage+solar_percetage+hydro_percentage

    data = {
        'Energía Renovable': ['Eólica', 'Solar', 'Hidroélectica'],'Participación': [wind_percetage, solar_percetage, hydro_percentage]
    }
